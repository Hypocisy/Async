...
public class ExecutorManager {
    ...
    private final EntityThread[] threads;
    ...
    public ExecutorManager(int workerThreadCount, Consumer<Thread> threadInitializer) {
        threads = new EntityThread[workerThreadCount];
        for (int i = 0; i < workerThreadCount; i++) {
            final EntityThread thread = new EntityThread(this);
    ...
    public void shutdown() {
        for (EntityThread workerThread : threads) {
    ...
    public void schedule(Runnable runnable, int priority) {
        this.schedule(new EntityTask(runnable, priority));
    }
